...
function populateMatchPool(matches) {
  ...
  matches.forEach(function(m) {
    ...
    window.matchPool.push({
      ...
      leagueId:    (m.leagueId !== undefined && m.leagueId !== null) ? m.leagueId : null,
    });
  });
...
// Compétitions réelles mais à fortement dépriorisée pour le Hero Live (pas une exclusion : elles restent
// dans le pool / Today, elles ne doivent simplement jamais gagner la vedette Live face à un match majeur).
var HOME_LOW_PRIORITY_RE = /\bnpc\b|\bd3\b|\bdivision 3\b|\biii\b|\breserves?\b|\bcaf\b/i;

// UCL — jamais un simple matching textuel "champions league" seul (leagueId=2 collisionne avec
// d'autres sports selon la source API-Sports, ex. NPB baseball). Gate strict : sport foot obligatoire,
// exclusions CAF / Championship anglais, puis nom explicite OU leagueId===2 uniquement en contexte foot.
function isUclMatch(m) {
  if (!m) return false;
  var sid = String(m.sportId || m.sport || '').toLowerCase();
  if (sid !== 'foot' && sid !== 'football' && sid !== 'soccer') return false;
  var comp = String(m.competition || '');
  if (/caf/i.test(comp)) return false;
  if (/championship/i.test(comp)) return false;
  var isExplicitUCL = /uefa champions league/i.test(comp);
  var lid = m.leagueId;
  var isLeagueId2 = (lid === 2 || lid === '2');
  return isExplicitUCL || isLeagueId2;
}
...
function computeMatchScore(m) {
  var score = 0;
  if (m.isLive) {
    var isMajorLive = isUclMatch(m) || HOME_PRIORITY_RE.test(m.competition || '');
    var isLowPrioLive = HOME_LOW_PRIORITY_RE.test(m.competition || '');
    if (isMajorLive) score += 1200;
    else if (isLowPrioLive) score += 150;
    else score += 700;
  }
