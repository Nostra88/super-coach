// ═══════════════════════════════════════════════════════════════════
// SUPERCOACH v9.3 — Moteur Quantitatif Multisports
...
async function callGemini(apiData, geminiKey) {
  ...
  for (const model of models) {
    try {
      ...
      // Confidence — ATTENTION : ceci n'est PAS une mesure de fiabilité indépendante.
      // C'est littéralement la probabilité de l'issue la plus probable, rebaptisée.
      // 72% de confidence == "le modèle donne 72% à son scénario principal", rien de plus.
      // Une vraie métrique de qualité (richesse des données, cohérence des sources,
      // stabilité entre plusieurs appels) reste à construire séparément — chantier "Fact Engine".
      ...
      try {
        result.markets = require('./goalMarkets').computeFootballMarkets(apiData, result);
      } catch (e) {
        result.markets = null;
      }
      return result;
...
/**
 * computeKellyAndValueEdge(geminiResult, userOdds, outcome)
 * Exécuté côté backend APRÈS réception du JSON. Jamais dans Gemini.
 *
 * @param {string} [outcome] 'home' | 'draw' | 'away' — l'issue à laquelle userOdds
 *   correspond réellement. Sans ce paramètre valide (home/draw/away), la fonction
 *   retourne null plutôt que de deviner — comparer une cote à la mauvaise issue
 *   serait plus trompeur que de ne pas afficher de Value du tout.
 */
function computeKellyAndValueEdge(geminiResult, userOdds, outcome) {
  if (!userOdds || userOdds <= 1.01) return null;
  if (geminiResult.error) return null;

  const fp = geminiResult.final_probability;
  const outcomeMap = { home: fp.home_win, draw: fp.draw || 0, away: fp.away_win };
  const hasValidOutcome = outcome && Object.prototype.hasOwnProperty.call(outcomeMap, outcome);

  // Sans issue précisée et valide, on refuse de calculer plutôt que de deviner (voir doc ci-dessus).
  if (!hasValidOutcome) return null;

  const targetProb = outcomeMap[outcome];
  ...
  return { valueEdgePct, kellyUnits, recommendation, outcomeUsed: outcome };
}
...
module.exports = {
  ...
  SPORTS,
  computeFootballMarkets: function (apiData, geminiResult) {
    return require('./goalMarkets').computeFootballMarkets(apiData, geminiResult);
  },
};
