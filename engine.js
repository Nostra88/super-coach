 ...
 *   correspond réellement. CRITIQUE : sans ce paramètre, impossible de garantir que
 *   la cote comparée correspond à la bonne probabilité (ex: cote de l'extérieur
 *   comparée par erreur à la probabilité du favori). Si absent/invalide, on retombe
 *   sur maxProb par compatibilité, mais le résultat n'est alors pas fiable.
 */
function computeKellyAndValueEdge(geminiResult, userOdds, outcome) {
  ...
  // Probabilité de l'issue réellement couverte par la cote fournie.
  // Fallback maxProb uniquement si l'issue n'est pas précisée (legacy, non fiable).
  const targetProb = hasValidOutcome ? outcomeMap[outcome]
                    : Math.max(fp.home_win, fp.draw || 0, fp.away_win);
  ...
  return { valueEdgePct, kellyUnits, recommendation, outcomeUsed: hasValidOutcome ? outcome : 'max_prob_fallback' };
}
