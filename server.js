  ...
  try {
    ...
    for (const model of MODELS) {
      try {
        ...
        text = candidate;
        usedModel = model;
        console.log(`[MODEL] ${model} utilisé avec succès`);
        break;
    ...
    res.json({
      result: text,
      meta: { sports: sportIds, live: sources.live.length, news: sources.news.length, fd: sources.fd.length, model: usedModel }
    });
