...
app.post('/admin/import-history', async (req, res) => {
  ...
  if (provided !== secret) return res.status(401).json({ error: 'Unauthorized.' });
  if (!sql) return res.status(503).json({ error: 'Neon non configuré (DATABASE_URL manquant).' });
  if (!APISPORTS_KEY) return res.status(503).json({ error: 'APISPORTS_KEY manquant.' });

  const { league, season } = req.body || {};
  if (!league || !season) return res.status(400).json({ error: 'Paramètres requis : league, season' });
  ...
  try {
    ...
    if (!fixtures.length) {
      return res.json({ league, season, fetched: 0, inserted: 0, updated: 0, message: 'Aucun match retourné par API-Football pour ce league/season — vérifier les paramètres.' });
    }

    // Construction des colonnes en parallèle, filtrage des lignes incomplètes.
    const cols = {
    ...
    if (!cols.fixtureId.length) {
      return res.json({ league, season, fetched: fixtures.length, inserted: 0, updated: 0, skipped, message: 'Toutes les lignes étaient incomplètes.' });
    }

    // Une seule requête pour toute la saison (au lieu d'une par match) — résout le timeout.
    ...
    console.error('[/admin/import-history] Erreur', e);
    res.status(500).json({ error: e.message });
  }
});
