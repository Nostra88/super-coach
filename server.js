...
// ── MOTEUR QUANTITATIF ──────────────────────────────────────────
const { GEMINI_SYSTEM_PROMPT, buildPrompt: buildEnginePrompt, computeKellyAndValueEdge, computeFootballMarkets } = require('./engine.js');
...
async function fetchESPN(url) {
  ...
  try {
    const wait = url.includes('tennis') ? 15000 : 4000;
    const ctrl=new AbortController(); setTimeout(()=>ctrl.abort(),wait);
    ...
    const matches = (j.events||[]).map(e=>{
      ...
      const nameOf = (c) => c?.team?.displayName || c?.athlete?.displayName || c?.athlete?.fullName || c?.displayName || '';
      const homeN = nameOf(home) || nameOf(teams[0]);
      const awayN = nameOf(away) || nameOf(teams[1]);
      ...
      const leagueName = (isTennis ? (j.leagues?.[0]?.name || e.name || 'Tennis') : (j.leagues?.[0]?.name || (typeof isWC!=='undefined' && isWC ? 'World Cup' : compName)));
    ...
    if (matches.length) cacheSet(url, matches);
    return matches;
  } catch(e) { return []; }
}


async function fetchEspnTennisDirect() {
  const tours = ['atp', 'wta'];
  const out = [];
  const seen = new Set();
  const now = Date.now();
  ...
  for (const tour of tours) {
    try {
      const listUrl = 'https://sports.core.api.espn.com/v2/sports/tennis/leagues/' + tour + '/events';
      const lr = await fetch(listUrl, { headers: { 'User-Agent': 'SUPERCOACH' } });
      if (!lr.ok) { console.log('[TENNIS] list', tour, lr.status); continue; }
      const list = await lr.json();
      const refs = (list.items || []).map(it => toHttps(it.$ref)).filter(Boolean);
      for (const ref of refs.slice(0, 4)) {
        const er = await fetch(ref, { headers: { 'User-Agent': 'SUPERCOACH' } });
        if (!er.ok) continue;
        const ev = await er.json();
        const leagueName = ev.name || ev.shortName || 'US Open';
        (ev.competitions || []).forEach((comp) => {
          const comps = comp.competitors || [];
          if (comps.length < 2) return;
          if (comps.some(c => c.winner === true)) return;
          const homeN = comps[0].name || '';
          const awayN = comps[1].name || '';
          if (!homeN || !awayN) return;
          const when = new Date(comp.date || ev.date || 0).getTime();
          if (!when) return;
          if (when < now - 16 * 3600 * 1000) return;
          if (when > now + 48 * 3600 * 1000) return;
          const key = homeN + '|' + awayN;
          if (seen.has(key)) return;
          seen.add(key);
          const live = !!(comp.liveAvailable) || (when <= now && when >= now - 6 * 3600 * 1000);
          out.push({
            id: 'espn_ten_' + (comp.id || key),
            sport: 'tennis',
            sportId: 'tennis',
            home: homeN,
            away: awayN,
            competition: leagueName + (comp.type && comp.type.text ? ' · ' + comp.type.text : ''),
      ...
      console.log('[TENNIS] core', tour, e.message);
    }
  }
  console.log('[TENNIS] core parsed', out.length);
  return out;
}
...
app.post('/analyze-match', rateLimit, async (req, res) => {
  ...
  try {
    ...
    let kelly = null;
    try {
      if (odds && geminiResult && geminiResult.final_probability) {
        kelly = computeKellyAndValueEdge(geminiResult, parseFloat(odds), oddsOutcome);
      }
    } catch (e) { kelly = null; }
    let markets = null;
    try {
      markets = computeFootballMarkets(apiData, geminiResult);
    } catch (e) { markets = null; }
    const { premium } = await getPremiumStatus(req);
...
app.get('/fixtures', async (req, res) => {
  ...
  try {
    ...
    // Tennis manquant (API tennis souvent en "fetch failed") → ESPN ATP/WTA
    const tennisCount = allMatches.filter(m => m.sport === 'tennis').length;
    if (tennisCount === 0) {
      console.log('[FIXTURES] Tennis vide — repli ESPN ATP/WTA');
      try {
        const espnTennis = await fetchEspnTennisDirect();
        espnTennis.forEach(ev => {
          const sportId = ev.sportId || 'tennis';
          if (sportId !== 'tennis') return;
          if (!ev.home || !ev.away) return;
          const live = !!ev.live;
          allMatches.push({
            id:          ev.id || `espn|${ev.home}|${ev.away}`,
            sport:       'tennis',
            emoji:       '🎾',
            home:        ev.home,
            away:        ev.away,
            homeLogo:    '',
            awayLogo:    '',
            competition: ev.competition || 'Tennis',
            country:     '',
            dateUTC:     ev.dateUTC || '',
            isLive:      live,
            isFinished:  false,
            score:       live && (ev.score_home || ev.score_away) ? `${ev.score_home}-${ev.score_away}` : '',
            statusShort: live ? 'LIVE' : 'NS',
            leagueId:    0,
          });
        });
        console.log('[FIXTURES] Tennis ESPN:', allMatches.filter(m => m.sport === 'tennis').length);
      } catch (e) {
        console.log('[FIXTURES] Tennis ESPN error:', e.message);
      }
    }
