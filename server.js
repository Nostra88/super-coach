...
// Cascade de modèles — modèles actifs au 27/05/2026
// gemini-2.0-flash retraité le 01/06/2026 — NE PAS UTILISER
const MODELS = [
  'gemini-2.5-flash',        // Primaire — génération actuelle
  'gemini-2.5-flash-lite',   // Fallback 1 — plus léger, même génération
];
...
app.post('/analyze', async (req, res) => {
  try {
    ...
    const sportIds   = detectIntention(prompt);
    const T1 = Date.now();
    console.log(`[TIMER] Intention détectée: ${T1-T0}ms — Sports: ${sportIds.join(', ')}`);

    // 2. Fetch ciblé multi-sources
    const sources  = await fetchAllData(sportIds);
    const T2 = Date.now();
    console.log(`[TIMER] Fetch sources: ${T2-T1}ms — Live:${sources.live.length} News:${sources.news.length} FD:${sources.fd.length}`);
    ...
    // Mesure taille du prompt envoyé à Gemini
    const promptTokenEstimate = Math.round(enrichedPrompt.length / 4);
    console.log(`[TIMER] Prompt construit: ${T3-T2}ms — Taille: ${enrichedPrompt.length} chars (~${promptTokenEstimate} tokens)`);

    // 4. Rappel format JSON — doux mais clair
    enrichedPrompt += '\n\nFORMAT RULES (mandatory):\n' +
    '1. Respond ONLY with a valid JSON object.\n' +
    '2. The root must be an object: {"matches":[...]} — NEVER a bare array [...].\n' +
    '3. No markdown, no backticks, no prose before or after.\n' +
    '4. Start with { and end with }.';

    // 5. Appel Gemini avec fallback automatique
    let text = null;
    let usedModel = null;
    let lastError = null;
    ...
    for (const model of MODELS) {
      try {
        ...
        // Timeout 25s par modèle — si trop lent, on essaie le suivant
        ...
        usedModel = model;
        const T4 = Date.now();
        console.log(`[TIMER] Gemini (${model}): ${T4-T3}ms — Réponse: ${text.length} chars`);
        console.log(`[TIMER] TOTAL pipeline: ${T4-T0}ms`);
        break;

      } catch (err) {
        if (err.name === 'AbortError') {
          console.log(`[FALLBACK] ${model} timeout 25s → modèle suivant`);
          lastError = 'timeout';
        } else {
          console.log(`[FALLBACK] ${model} erreur → ${err.message}`);
    ...
    if (!text) {
      return res.status(503).json({
        error: `Tous les modèles Gemini sont temporairement indisponibles. Réessaie dans quelques minutes. (${lastError})`
      });
    }
...
// Warmup au démarrage
// ═══════════════════════════════════════════════════════════
// HEALTH CHECK MODÈLES GEMINI
// Teste chaque modèle au démarrage — alerte si déprécié
// ═══════════════════════════════════════════════════════════
async function checkModelsHealth() {
  ...
  for (const model of MODELS) {
    try {
      ...
      if (r.status === 404) {
        console.error(`[MODEL CHECK] ❌ ${model} — INTROUVABLE (déprécié ou retiré)`);
        console.error(`[MODEL CHECK] ⚠️  Retirer ${model} de la cascade IMMÉDIATEMENT`);
      } else if (r.status === 429) {
        console.warn(`[MODEL CHECK] ⚠️  ${model} — Quota atteint (mais modèle actif)`);
      } else if (r.ok) {
        console.log(`[MODEL CHECK] ✅ ${model} — Opérationnel`);
      } else {
        console.warn(`[MODEL CHECK] ⚠️  ${model} — HTTP ${r.status}`);
      }
    } catch (err) {
      if (err.name === 'AbortError') {
        console.warn(`[MODEL CHECK] ⚠️  ${model} — Timeout (surchargé mais actif)`);
      } else {
        console.error(`[MODEL CHECK] ❌ ${model} — Erreur: ${err.message}`);
      }
    }
  }
}

// Endpoint de statut — consultable depuis Render dashboard
app.get('/health', async (req, res) => {
  ...
  for (const model of MODELS) {
    try {
      ...
      status[model] = e.name === 'AbortError' ? '⚠️ Timeout' : `❌ ${e.message}`;
    }
  }
...
app.listen(PORT, () => {
  ...
  // Warmup cache + vérification modèles au démarrage
  setTimeout(() => {
    const warmup = [
    ...
    // Health check modèles (si clé disponible)
