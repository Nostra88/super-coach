    ...
    if (APISPORTS_KEY && apiData.sport === 'football') {
      try {
        ...
        if (match) {
          ...
          apiData.homeLastMatchHoursAgo = null;
          apiData.awayLastMatchHoursAgo = null;
          async function fetchFormAndFatigue(teamId) {
            ...
            const played = (data?.response || []).filter(f => f.fixture?.status?.short === 'FT');
            if (!played.length) return { form: null, hoursAgo: null };
            ...
            const mostRecent = played[0]?.fixture?.date; // API renvoie du plus récent au plus ancien
            const hoursAgo = mostRecent ? Math.round((Date.now() - new Date(mostRecent).getTime()) / 3600000) : null;
            return { form, hoursAgo };
          }
          try {
            ...
            apiData.homeLastMatchHoursAgo = homeStats.hoursAgo;
            apiData.awayLastMatchHoursAgo = awayStats.hoursAgo;
            console.log(`[/analyze-match] Forme: ${apiData.home}=${apiData.homeForm ?? '?'} (${apiData.homeLastMatchHoursAgo ?? '?'}h) | ${apiData.away}=${apiData.awayForm ?? '?'} (${apiData.awayLastMatchHoursAgo ?? '?'}h)`);
          } catch(e) {
            ...
            if (city && kickoff) {
              ...
              if (loc) {
                ...
                if (idx >= 0) {
                  ...
                  apiData.weatherCondition = condition;
                  apiData.weatherTempC = wData.hourly.temperature_2m[idx];
                  apiData.weatherWindKmh = wData.hourly.wind_speed_10m[idx];
                } // sinon : hors fenêtre de prévision (trop loin dans le futur) → reste null
              }
            }
            console.log(`[/analyze-match] Météo: ${apiData.weatherCondition ?? 'indisponible'} (ville: ${city ?? 'inconnue'})`);
          } catch(e) {
