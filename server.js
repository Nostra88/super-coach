...
// Détecte si un texte contient une date plausiblement future/actuelle, plutôt que de se fier
// à des mots-clés vagues ('vs', 'odds', l'année courante) qui laissent passer du contenu périmé.
// Année dynamique (jamais codée en dur) : ne se périme pas avec le temps.
function hasPlausibleFutureDate(text) {
  const years = [...text.matchAll(/\b(19|20)\d{2}\b/g)].map(m => parseInt(m[0], 10));
  if (!years.length) return true; // aucune année détectée → signal insuffisant pour trancher seul
