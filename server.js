...
function buildContainer(entities, validation, espnLines, prompt) {
  ...
  if (validation.status==='VERIFIED' && validation.events?.length) {
    b += '\n[ODDS — '+validation.source+']\n';
    b += 'USE THESE ODDS ONLY FOR value_edge_pct CALCULATION — never as prediction guide.\n';
    validation.events.slice(0,5).forEach(ev => {
...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    const entities = await extractEntitiesWithGemini(prompt).catch(()=>[]);
    ...
    const [validation, espnLines] = await Promise.all([
      validateViaOddsAPI(entities, isWebMode).catch(()=>({status:'ENRICHMENT_ONLY',entities})),
    ...
    try {
      ...
      const gr=await fetch(
        'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key='+GEMINI_KEY,
        {
          ...
          body:JSON.stringify({
            contents:[{role:'user',parts:[{text:
              'Search for current sports matches and odds on this website: '+parsed.hostname+'\n'+
              'URL: '+url+'\n'+
              'Extract ONLY matches from '+parsed.hostname+'. Do NOT use ESPN or other sources.\n'+
              'Return structured list: team1 vs team2 | competition | date | odds\n'+
              'If inaccessible, return: BLOCKED'
            }]}],
            tools:[{google_search:{}}],
